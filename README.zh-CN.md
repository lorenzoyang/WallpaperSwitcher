...
在 **自定义模式** 下，每次通过按钮、托盘菜单或快捷键执行 **下一张壁纸**，都会先重新扫描当前文件夹。新增图片会按文件名顺序加入列表，已删除图片会移出列表，并从当前图片的位置继续切换。仅扫描所选文件夹内直接存放的受支持图片，不包含子文件夹，无需重启或手动刷新。

暂时无法设置的图片（例如仍在复制中的文件）会在本次切换中跳过，后续切换时仍可重试。如果文件夹为空、暂时无法访问，或没有其他可用图片，则保持当前桌面壁纸；下次执行 **下一张壁纸** 时会重新尝试所选文件夹。
