...
托盘菜单采用与主窗口一致的配色和字体，配有可缩放的线性图标，并以勾选标记当前文件夹。以下为使用示例文件夹的菜单预览：

<img src="./assets/TrayMenu-Folders.png" alt="壁纸切换器托盘菜单及文件夹子菜单" width="720"/>
...
- 可在 **设置** 中修改快捷键
- 如需录制快捷键，点击 **Modify**，然后直接按下受支持的 `Ctrl`/`Alt`/`Shift`/`Win` + `A`-`Z` 组合键
- 也可以在同一个输入框中手动输入快捷键
- 录制或手动输入的修改仅在点击 **Save** 后生效
- 编辑时按 `Esc` 可取消修改并恢复之前的值
- 清空输入框后点击 **Save** 可禁用该快捷键绑定
- 使用 `+` 作为分隔符；空格和大小写会被忽略
